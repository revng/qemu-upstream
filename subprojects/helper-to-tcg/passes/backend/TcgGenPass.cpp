...
class Mapper {
  ...
  public:
    ...
    Expected<TcgV> mapCondAndEmit(Value *V, uint32_t TcgSize,
                                  uint32_t LlvmSize) {
        auto Mapped = getMapped(V);
        if (Mapped) {
...
static bool translatePseudoInstCall(raw_ostream &Out, CallInst *Call,
    ...
    case Movcond: {
        auto LlvmPred = static_cast<ICmpInst::Predicate>(
            cast<ConstantInt>(Call->getOperand(0))->getZExtValue());
        if (Args[1].Kind == IrImmediate && Args[2].Kind == IrImmediate &&
            Args[3].Kind == IrImmediate && Args[4].Kind == IrImmediate) {
    ...
    case JumpPCRelConditional: {
        Out << "xqci_jump_conditional_pcrel" << "(";
        if (ForwardContext) {
            Out << "ctx, ";
        }
        Out << Args[0];
    ...
    case JumpPCRelFallthrough: {
        Out << "xqci_jump_fallthrough_pcrel" << "(";
        if (ForwardContext) {
            Out << "ctx";
        }
        Out << ");\n";
            ...
            case Instruction::Call: {
                ...
                if (F->isIntrinsic()) {
                    ...
                    if (!F->isDeclaration()) {
                        ...
                        Expected<TcgV> MaybeRes =
                            mapCallReturnValue(Mapper, Call);
                        ...
                        // Call to declaration
                        Expected<TcgV> MaybeRes =
                            mapCallReturnValue(Mapper, Call, true);
                        if (MaybeRes) {
                        ...
                        // Use function annotation data to force type of
                        // arguments
                        // TODO(anjo): Close to copy of code from
...
PreservedAnalyses TcgGenPass::run(Module &M, ModuleAnalysisManager &MAM) {
    ...
    if (OutputDispatcher) {
        ...
        OutSource
            << "int helper_to_tcg_dispatcher(void *func, TCGTemp *ret_temp, "
               "int nargs, TCGTemp **args) {\n";
        for (auto &TF : TranslatedFunctions) {
