...
public:
  ...
  bool hasBeenDefined(const TcgV &V) {
    return HasBeenDefined[V.Id];
  }

  void define(const TcgV &V) {
    HasBeenDefined.set(V.Id);
  }
  ...
  Expected<TcgV> mapAndEmit(const Value *V, bool ForceNondef = false) {
    ...
    if (ConstV and V->getType()->isVectorTy()) {
      ...
      if (Splat) {
        ...
        auto Size =
            TcgV::makeImmediate(Twine(vectorSizeInBytes(Tcg)).str(), 64, 64);
        ...
        if (PatternLen > 0) {
          ...
          auto Splat = TcgV::makeImmediate(Twine(Value).str(), 64, 64);
          auto Size =
              TcgV::makeImmediate(Twine(vectorSizeInBytes(Tcg)).str(), 64, 64);
...
static void ensureSignBitIsSet(raw_ostream &Out, const TcgV &V) {
  ...
  tcg::genExtract(
      Out, true, V, V, TcgV::makeImmediate("0", V.TcgSize, V.LlvmSize),
      TcgV::makeImmediate(Twine((int)V.LlvmSize).str(), V.TcgSize, V.LlvmSize));
}

static Expected<TcgV> mapCallReturnValue(Mapper &Mapper, CallInst *Call, bool CallToDecl = false) {
  ...
  return Mapper.mapAndEmit(Call, CallToDecl);
}
...
static bool translatePseudoInstCall(raw_ostream &Out, CallInst *Call,
  ...
  case Movcond: {
    ...
    if (Args[1].Kind == IrImmediate &&
        ...
        Args[4].Kind == IrImmediate) {
      const TcgV Cond = c::compare(LlvmPred, Args[1], Args[2]);
      const TcgV Ternary = c::ternary(Cond, Args[3], Args[4]);
      Mapper.mapExplicitly(Call, Ternary);
    } else {
      Expected<TcgV> MaybeRes = mapCallReturnValue(Mapper, Call);
      if (!MaybeRes) {
        return false;
  ...
  case GuestLoad: {
    ...
    tcg::genQemuLoad(Out, *MaybeRes, tcg::materialize(Args[0]),
                     MemOpStream.str().c_str());
  } break;
  case GuestStore: {
    uint8_t Size = cast<ConstantInt>(Call->getOperand(2))->getZExtValue();
    uint8_t Endianness = cast<ConstantInt>(Call->getOperand(3))->getZExtValue();
...
static bool translateIntrinsicCall(raw_ostream &Out, CallInst *Call,
 ...
 case Intrinsic::usub_sat: {
    ...
    if (Args[0].Kind == IrPtrToOffset) {
      tcg::genVecUnsignedSatSub(Out, *MaybeRes, Args[0], Args[1]);
    } else {
      tcg::genUnsignedSatSub(Out, *MaybeRes, Args[0], Args[1]);
    }
  } break;
  case Intrinsic::ctlz: {
    Expected<TcgV> MaybeRes = mapCallReturnValue(Mapper, Call);
...
translateFunction(const Function *F, const TcgGlobalMap &TcgGlobals,
  ...
  Expected<TempAllocationData> MaybeTAD = allocateTemporaries(*F, Annotations);
  if (!MaybeTAD) {
    ...
    for (Instruction &I : BB) {
      ...
      case Instruction::Br: {
        ...
        if (Branch->isConditional()) {
          ...
          tcg::genBrcond(Out, CmpInst::Predicate::ICMP_NE, CCondition, Zero,
                         True);
          tcg::genBr(Out, False);
        } else {
      ...
      case Instruction::ZExt: {
        ...
        if (SrcVal.get().Kind == IrImmediate) {
          ...
          Mapper.mapExplicitly(&I, c::zext(SrcVal.get(), ResLlvmSize,
                                           llvmToTcgSize(ResLlvmSize)));
          break;
        }
        ...
        if (Res.get().Kind == IrValue) {
          if (SrcVal.get().TcgSize == 64) {
            if (Res.get().LlvmSize == 32) {
              ...
              auto Offset =
                  TcgV::makeImmediate("0", MRes.TcgSize, MRes.LlvmSize);
              auto Size = TcgV::makeImmediate(Twine((int)MRes.LlvmSize).str(),
      ...
      case Instruction::Xor: {
        ...
        if (isa<IntegerType>(Bin->getType())) {
          if (Op1.Kind == IrImmediate and Op2.Kind == IrImmediate) {
            Mapper.mapExplicitly(Bin, c::binop(Bin->getOpcode(), Op1, Op2));
          } else {
            ...
            tcg::genBinOp(Out, Res.get(), Bin->getOpcode(), AOp1.get(ResSize),
                          AOp2.get(ResSize));
          }
          ...
          tcg::genVecBinOp(Out, Bin->getOpcode(), Res.get(), Op1, Op2);
        }
      } break;
      case Instruction::Call: {
        ...
        if (Name == "__assert_fail" or Name == "g_assertion_message_expr" or
            ...
            isa<DbgDeclareInst>(I) or (F->isIntrinsic() and
        ...
        if (F->isIntrinsic()) {
          ...
          if (!F->isDeclaration()) {
            ...
            // Call to declaration
            Expected<TcgV> MaybeRes = mapCallReturnValue(Mapper, Call, true);
            if (MaybeRes) {
              if (!Mapper.hasBeenDefined(*MaybeRes)) {
                Out << tcg::getType(*MaybeRes) << " ";
                Mapper.define(*MaybeRes);
              }
              Out << *MaybeRes << " = ";
            }
            Out << getDemangleFunctionName(Name) << "(";
            ...
            // TODO(anjo): Close to copy of code from TcgTempAllocationPass
            SmallVector<bool, 4> ArgIsImm(Args.size(), false);
            auto It = Annotations.find(F);
            if (It != Annotations.end()) {
                for (const Annotation &Ann : It->second) {
                    if (Ann.Kind == AnnotationKind::Immediate) {
      ...
      case Instruction::ICmp: {
        ...
        if (Op1.get().Kind == IrImmediate and Op2.get().Kind == IrImmediate) {
          Mapper.mapExplicitly(
              ICmp, c::compare(ICmp->getPredicate(), Op1.get(), Op2.get()));
          break;
        }
        ...
        if (Op1.get().Kind == IrPtrToOffset) {
          Expected<TcgV> Res =
              Mapper.mapCondAndEmit(&I, Op1.get().TcgSize, Op1.get().LlvmSize);
          if (!Res) {
            return mkError("couldn't map icmp result", &I);
          }
          tcg::genVecCmp(Out, Res.get(), LlvmPred, Op1.get(), Op2.get());
        } else {
          ...
          if (Op2.get().Kind == IrImmediate) {
            tcg::genSetcondI(Out, LlvmPred, Res.get(), IOp1, Op2.get());
          } else {
            tcg::genSetcond(Out, LlvmPred, Res.get(), IOp1, Op2.get());
          }
        }
      ...
      case Instruction::Select: {
        ...
        if (Res.get().Kind == IrPtr) {
          return mkError("Select statements for pointer types not supported: ",
                         Select);
        }
        ...
        if (!False) {
          return mkError("Couldn't map False for select instruction: ", Select);
        }
        ...
        if (Res.get().Kind == IrPtrToOffset) {
          ...
          tcg::genMov(Out, Res.get(), c::ternary(Cond.get(), MTrue, MFalse));
        } else {
          ...
          if (True.get().Kind == IrImmediate or
            ...
            tcg::genMovcond(Out, CmpInst::Predicate::ICMP_NE, Res.get(),
                            ACond.get(CTrue.TcgSize), Zero, CTrue, CFalse);
          } else {
            tcg::genMovcond(Out, CmpInst::Predicate::ICMP_NE, Res.get(),
      ...
      case Instruction::Load: {
        ...
        case IrImmediate: {
          ...
          tcg::genMovI(
              Out, Res.get(),
              c::deref(Mapped.get(), Res.get().LlvmSize, Res.get().TcgSize));
        } break;
        case IrValue: {
          Expected<TcgV> Res = Mapper.mapAndEmit(Load);
          if (!Res) {
      ...
      case Instruction::Switch: {
        ...
        const TcgV DefaultLabel =
            Mapper.mapBbAndEmit(cast<BasicBlock>(Switch->getOperand(1)));
        for (uint32_t i = 2; i < Switch->getNumOperands(); i += 2) {
          ...
          const TcgV BranchLabel =
              Mapper.mapBbAndEmit(cast<BasicBlock>(Switch->getOperand(i + 1)));
...
PreservedAnalyses TcgGenPass::run(Module &M, ModuleAnalysisManager &MAM) {
  ...
  for (Function &F : M) {
    ...
    for (auto *N : make_range(po_begin(Node), po_end(Node))) {
      ...
      auto Translated =
          translateFunction(F, TcgGlobals, Annotations, HasTranslatedFunction);
      if (!Translated) {
